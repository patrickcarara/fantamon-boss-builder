# Fantamon Boss Builder v2.3 Web

Esta versão foi preparada para publicação como site estático.

## Arquivos para publicar
Envie todo o conteúdo desta pasta para o repositório, mantendo:
- index.html
- script.js
- style.css
- pasta assets
- .nojekyll

## GitHub Pages
1. Crie um repositório no GitHub.
2. Envie todos os arquivos desta pasta para a raiz do repositório.
3. Abra Settings > Pages.
4. Em Build and deployment, selecione Deploy from a branch.
5. Selecione a branch main e a pasta / (root).
6. Salve e aguarde a publicação.

## Dados dos usuários
Bosses, estratégias e skills adicionadas ficam salvos localmente no navegador de cada pessoa.
Para transferir dados entre computadores ou navegadores, use Exportar Backup e Importar Backup.

## Atualizações
Para atualizar o site depois, substitua os arquivos publicados pelos arquivos da nova versão.


## Atualização v2.4 Web

- Removidas as 5 skills provisórias que vinham embutidas na versão inicial.
- A biblioteca agora começa vazia e usa apenas as skills adicionadas pelo usuário.
- Todas as skills adicionadas pelo usuário continuam com botão `×` para exclusão.
- Isso evita imagens quebradas quando o site é publicado no GitHub Pages.


## Atualização v2.5 Web
- Removidos fisicamente os 5 arquivos provisórios da pasta `assets/skills/mage`.
- A pasta `mage` permanece vazia apenas para manter a estrutura do projeto.

## Atualização v2.6 Web

- Corrigido `data/classes/mage.json`.
- Removidas as referências às 5 skills provisórias que já não existem mais em `assets/skills/mage`.
- O arquivo de Mage agora contém uma lista vazia de skills.
